...
cv.onRuntimeInitialized=()=>{
  ...
  for(const name of ['464','465']){
    ...
    const tiles=D._test.findTiles(src,0.015,0.70,0.03);
    console.log(`\n=== Photo ${name}: ${tiles.length} tiles fills=[${tiles.map(t=>t.fill.toFixed(3)).join(',')}] ===`);
    ...
    for(let ti=0;ti<tiles.length;ti++){
      ...
      const halves=landscape?[['L',cropped.roi(new cv.Rect(0,0,mid,cropped.rows))],['R',cropped.roi(new cv.Rect(mid,0,cropped.cols-mid,cropped.rows))]]:
        [['T',cropped.roi(new cv.Rect(0,0,cropped.cols,mid))],['B',cropped.roi(new cv.Rect(0,mid,cropped.cols,cropped.rows-mid))]];
      for(const [lbl,halfRoi] of halves){
        ...
        for(let i=0;i<conts.size();i++){
          const c=conts.get(i);const area=cv.contourArea(c);const peri=cv.arcLength(c,true);c.delete();
        ...
        if(pipAreas.length>=8&&pipAreas.length<10){
          if(pipAreas.length===9&&bigBlobs.length>=2){
            ...
            for(const ba of bigBlobs){
              if(ba>=pipArea*2.5){count+=Math.round(ba/pipArea);if(pipAreas.length===8)count+=1;}
              else if(ba>=pipArea*1.5&&pipAreas.length===8)count+=1;
            }
          }
        }
        count=Math.min(count,12);
        console.log(`  T${ti}${lbl}: accepted=${pipAreas.length} bigBlobs=[${bigBlobs.map(b=>b.toFixed(0)).join(',')||'none'}] blobs≥2.5x=[${bigBlobs.filter(b=>b>=pipArea*2.5).map(b=>b.toFixed(0)).join(',')||'none'}] pipArea=${pipArea.toFixed(0)} count=${count}`);
