// Headless accuracy harness — runs the REAL detect.js pip pipeline against the
// reference photos and scores against fixtures. No API, no browser.
//   npm install   (once, pulls @techstark/opencv-js + jpeg-js)
//   node headless.cjs
const fs = require("fs");
...
cv.onRuntimeInitialized = () => {
  // Minimal DOM shim so detect.js's IIFE can attach window.DominoCV.
  global.cv = cv;
  global.window = {};
  ...
  console.log(`\nTOTAL: ${totC}/${totE} tiles · ${perfect}/${Object.keys(FIX).length} photos perfect`);
  process.exit(0);
};
