  ...
  for(const name of ['467','468','469']){
    ...
    for(let ti=0;ti<tiles.length;ti++){
      ...
      for(const [lbl,halfRoi] of halves){
        ...
        console.log(`  T${ti}${lbl}: accepted=${pipAreas.length} bigBlobs=[${bigBlobs.map(b=>b.toFixed(0)).join(',')||'none'}] blobs≥2.5x=[${blobs25.map(b=>b.toFixed(0)).join(',')||'none'}] pipArea=${pipArea.toFixed(0)} count=${count}`);
