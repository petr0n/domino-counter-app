...
**Shipped counter — `countPips` → `countPipsContour`:** before binarising, a **warmth pre-pass** (on the RGBA half-tile clone, inside `countPipsContour` only — NOT in `DominoCV.preprocess`) darkens yellow/orange pixels: `warmth = max(0, min(R,G) - B)`, then `R *= f`, `G *= f` where `f = 1 - 0.4*(warmth/255)`. This is zero for white, gray, red, purple, blue — so those pip colors are unaffected. It runs only on the half-tile crop, so it cannot affect tile detection. Then each half is binarised **two ways** and the **higher pip count wins** (both methods only ever *under*-count in their failure modes, so the larger is the reliable one — and there is no contrast threshold to tune, which mattered because `mean−min` is contaminated by the dark divider bar and so can't separate faint pips from bold ones):
- **(A) large-block adaptive threshold** — a local cut relative to background; robust for normal/bold pips of any colour or count.
- **(B) black-hat + fixed cut** — the black-hat measures each pip vs its *local* surround, so faint pips (e.g. yellow on white, barely darker than the tile) become strong blobs that a fixed adaptive offset (C) would miss. Kernel must exceed the pip diameter so the closing fills each pip solid. This is the low-contrast pass that fixed the bright-background `6|6` (`scan_eval` 6/9 → 9/9).

Each binarisation is counted by the shared `countBlobs`: external contours, round blobs (area in range AND circularity ≥ 0.45) are pips; remaining low-circularity blobs (circ 0.15–0.45; thinner slivers / divider bars at circ ~0.1 excluded — that floor fixed `ref_464` 6→7) are split **by area** — a blob ≈1× mean pip area (0.55–1.6×) **and compact (aspect < 2.2)** is one distorted pip counted at any total (the aspect gate keeps a black-hat-fattened divider sliver from being miscounted, `9→10`; the area rule fixed the `20260614` batch's `3→2` / `6→5` / `9→8`); larger blobs (>1.6×) are genuine fusions added by the guarded dense-grid estimator only when 6–9 pips are already found. The layouts above are what the counter must reproduce — its job is to land on the right 0–12 total per half.
...
### Tile Detection Code Is Locked — Do Not Touch Without an Explicit Ask
**`findTiles`, `dividerSplit`, `dividerScan`, `pipClusterScan`, `padAndRotateCrop`, `cropRotate`, and the edge-margin logic in `detect.js` are locked.** Do not touch any of this code unless the user explicitly asks for a tile detection change. Default work in scope is pip counting.

`findTiles` runs the bright-blob (Otsu) path first. Three refinements then make it robust:
- **Stacked-tile split (divider count):** a blob in the single-tile aspect range (1.3–3.0) can actually be several tiles stacked with small gaps that erosion didn't separate (3 landscape tiles ≈ 3:2, 4 ≈ 2:1 — shape alone can't tell them from one tile). Each domino has exactly **one** centre divider bar, so the blob is passed to `dividerSplit`: if it holds ≥2 dividers it is ≥2 tiles, recovered individually. This fixed `detect_stack4` (top 3 tiles fused into one blob, was counted as a single garbage tile).
- **Bright-background recovery (`dividerScan`):** when tiles fuse into a bright surface (white towel, sun-glare) Otsu under-detects (often 0). A black-hat isolates dark divider bars across the whole frame and reconstructs each tile via the 2:1 model, validated by size-consistency and overlap. Its tiles are **merged** into the bright-blob result (not chosen over it): each divider-scan tile is added only if its centre doesn't fall inside an already-found tile (and vice-versa). Merging — rather than the old "prefer whichever finds more" — recovers tiles the bright path missed without letting divider-scan's occasional false positive override a correct clean-background result.
- **White-on-white recovery (`pipClusterScan`):** when the tile is as bright as the background (tiles on a white cloth) `dividerScan`'s brightness gate also fails — a densely-pipped tile's mean falls below the gate and is, per-candidate, indistinguishable from a bright gap-bar that catches a neighbour's pips. The fix is **holistic**: detect round pips (multi-scale black-hat + Otsu, sized from the tiles' own `medL` so the kernel exceeds pip diameter) and divider bars, then **assign every pip to exactly one bar — the containing bar whose centre is nearest**. A bar is a tile only if it **owns ≥4 pips AND ≥60% of the pips in its footprint** (a gap-bar owns only a fraction; the rest sit closer to the real tiles it spans). Also **additive + overlap-suppressed**, so it only recovers tiles the other two paths miss and cannot regress a clean result. Recovers the count on `detect_cloth4`. **Known limitation (next step):** the tile *rectangle* is taken from the selected bar, so a tile picked via a skewed bar can get a mis-oriented crop — fine for the tile-count gate, but tile geometry should be re-derived from the owned pip cluster before this path's pip *counts* are trusted.

Verified by `detect_eval.cjs` (6/6: `detect_4tiles`, the bright-background `detect_towel3` / `detect_glare3` / `detect_grass3`, the stacked `detect_stack4`, and the white-cloth `detect_cloth4`).
...
3. `minAreaRect` per blob, filtered by area, aspect ratio, and fill ratio → tile rectangles (tolerant of rounded corners and rotation). A blob that matches neither a single tile nor the geometric two-tile split (e.g. tiles touching at an angle) is passed to `dividerSplit`, which finds each tile's centre divider bar inside the blob and reconstructs the full rectangle from it via the 2:1 model (short side = divider length, long side = 2×, perpendicular). This is additive — such blobs were previously discarded. If the whole bright-blob pass under-detects (bright/cluttered background fuses tiles into the surface), `findTiles` falls back to `dividerScan` (black-hat → divider-bar reconstruction across the whole frame); see the tile-detection lock section.
4. Perspective warp to isolate each tile, detect the centre divider bar by column/row scan (`findDividerCol`/`findDividerRow`), split into halves at the detected divider instead of a fixed midpoint
5. Two binarisations per half — large-block adaptive threshold **and** black-hat+fixed-cut (low-contrast pass) — each counted by `countBlobs` (round-contour + compact-aspect area-split recovering distorted single pips and adding fused blobs via a guarded estimator); the **higher count wins** → pip count (see "Pip grid model" above). A lattice/occupancy counter (`countPipsGrid`) exists but is dormant.
...
Internally the scan functions share the same primitives: `findTiles` (with `dividerSplit` to recover touching tiles, a `dividerScan` fallback for bright/cluttered backgrounds, and a `pipClusterScan` fallback for white-on-white tiles) → `cropRotate` / `perspectiveWarp` → `splitRect` / `getHalvesMats` → `countPips` (currently the contour counter `countPipsContour`; `countPipsGrid` / `countPipsGridSample` are present but dormant — see "Pip grid model"). Keep this singular — do not duplicate detection logic into pages.
...
- **DETECTION gate — `node detect_eval.cjs` (free, local):** `grid_eval` feeds pre-cropped single tiles to the counter and never exercises `findTiles`; this runs the real `preprocess` + `findTiles` on committed multi-tile photos (`detect_4tiles.jpg`; the bright-background `detect_towel3` / `detect_glare3` / `detect_grass3` that exercise the `dividerScan` fallback; and the white-cloth `detect_cloth4.jpg` that exercises `pipClusterScan`) and checks the detected tile count. Run it after any `findTiles` / `dividerSplit` / `dividerScan` / `pipClusterScan` change.
