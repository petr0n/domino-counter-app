...
`findTiles` runs the bright-blob (Otsu) path first; when that **under-detects on a bright/cluttered background** (tiles fuse into a white towel, sun-glare, etc., so Otsu can't separate them) it falls back to **`dividerScan`** — a black-hat that isolates dark divider bars on bright tiles and reconstructs each tile via the 2:1 model, validated by size-consistency and overlap. The fallback is only preferred when it finds strictly more tiles than the bright-blob path, so a correct bright-blob result is never overridden. Verified by `detect_eval.cjs` (4/4: `detect_4tiles` plus the bright-background `detect_towel3` / `detect_glare3` / `detect_grass3`); the controlled full-res phone photos detect identically with and without the fallback.
