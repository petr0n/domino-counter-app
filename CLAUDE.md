...
**`findTiles`, `dividerSplit`, `dividerScan`, `padAndRotateCrop`, `cropRotate`, and the edge-margin logic in `detect.js` are locked.** Do not touch any of this code unless the user explicitly asks for a tile detection change. Default work in scope is pip counting.

`findTiles` runs the bright-blob (Otsu) path first. Two refinements then make it robust:
...
Verified by `detect_eval.cjs` (5/5: `detect_4tiles`, the bright-background `detect_towel3` / `detect_glare3` / `detect_grass3`, and the stacked `detect_stack4`).
...
Internally the scan functions share the same primitives: `findTiles` (with `dividerSplit` to recover touching tiles, and a `dividerScan` fallback for bright/cluttered backgrounds) → `cropRotate` / `perspectiveWarp` → `splitRect` / `getHalvesMats` → `countPips` (currently the contour counter `countPipsContour`; `countPipsGrid` / `countPipsGridSample` are present but dormant — see "Pip grid model"). Keep this singular — do not duplicate detection logic into pages.
...
- **DETECTION gate — `node detect_eval.cjs` (free, local):** `grid_eval` feeds pre-cropped single tiles to the counter and never exercises `findTiles`; this runs the real `preprocess` + `findTiles` on committed multi-tile photos (`detect_4tiles.jpg`; plus the bright-background `detect_towel3` / `detect_glare3` / `detect_grass3` that exercise the `dividerScan` fallback) and checks the detected tile count. Run it after any `findTiles` / `dividerSplit` / `dividerScan` change.
