...
**Shipped counter — `countPips` → `countPipsContour`:** large-block **adaptive**-threshold each half (a local threshold, so pips of any colour — including raised pips the same colour as the tile — separate from their background where a global Otsu cut fails), find external contours, and keep round blobs (area in range AND circularity ≥ 0.45) as pips. Remaining low-circularity blobs (circ 0.15–0.45; thinner slivers / divider bars at circ ~0.1 are excluded — that floor fixed the `ref_464` 6→7 overcount) are split **by area**: a blob ≈1× the mean pip area (0.55–1.6×) is one real pip with a distorted outline (faint shadow rim or slight edge-merge) and is counted as a single pip **at any total** — this fixed the `20260614` batch's `3→2` / `6→5` / `9→8` single-pip undercounts; larger blobs (>1.6×) are treated as genuine fused pips and added by the guarded dense-grid estimator, and then only when 6–9 individual pips are already detected. Noise/slivers measure ≤0.3× and stay below the 0.55× unit floor, so full halves are untouched. The layouts above are what the counter must reproduce — its job is to land on the right 0–12 total per half.
...
5. Large-block adaptive threshold → round-contour (area + circularity) pip detection, plus an area-split of low-circularity blobs that recovers distorted single pips (≈1× pip area) and adds genuine fused blobs (>1.6×) via a guarded estimator → pip count (see "Pip grid model" above). A lattice/occupancy counter (`countPipsGrid`) exists but is dormant.
