...
- [ ] **Step 1: Write the calibration-set builder script**
...
- [ ] **Step 3: Verify both dataset.yaml files load correctly**
...
- Produces: `runs/pose/touch/weights/best.onnx`, `runs/detect/pipdet_hn/weights/best.onnx`
...
```
tile detector exported: runs/pose/touch/weights/best.onnx
pip detector exported: runs/detect/pipdet_hn/weights/best.onnx
```
...
- [ ] **Step 3: Verify output shapes and value ranges via onnxruntime**
...
for path, expected_shape in [
    ("runs/pose/touch/weights/best.onnx", [1, 17, 8400]),
    ("runs/detect/pipdet_hn/weights/best.onnx", [1, 5, 3024]),
]:
...
- [ ] **Step 1: Write the photo-selection + reference-generation script**
...
- Consumes: `scanner/index.js` (Task 7), `onnxruntime-web` (CDN),
  `runs/pose/touch/weights/best.onnx` + `runs/detect/pipdet_hn/weights/best.onnx` (Task 2)
...
let ready = init({
  tileModelUrl: 'runs/pose/touch/weights/best.onnx',
  pipModelUrl: 'runs/detect/pipdet_hn/weights/best.onnx',
...
- [ ] **Step 2: Serve it locally and smoke-test**
